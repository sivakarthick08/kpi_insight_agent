...
export async function insertKPI(data: {
  name: string;
  description?: string;
  formula: string;
  ...
  if (clientType === 'postgres') {
    await pool.query(
      `INSERT INTO kpi (name, description, formula, table_name, columns) VALUES ($1, $2, $3, $4, $5);`,
      [data.name, data.description || null, data.formula, data.table_name || null, data.columns ? JSON.stringify(data.columns) : null]
    );
  } else {
    await pool.execute(
      `INSERT INTO kpi (name, description, formula, table_name, columns) VALUES (?, ?, ?, ?, ?);`,
      [data.name, data.description || null, data.formula, data.table_name || null, data.columns ? JSON.stringify(data.columns) : null]
    );
  }
}
...
export async function listTables(): Promise<string[]> {
  if (clientType === 'postgres') {
    ...
    `);
    return res.rows.map((r: any) => r.table_full_name);
  } else {
    const [rows] = await pool.query(`
      ...
      ORDER BY TABLE_SCHEMA, TABLE_NAME;
    `);
    return (rows as any[]).map((r) => r.table_full_name);
  }
}
